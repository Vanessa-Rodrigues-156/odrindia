...
import { ArrowLeft, Calendar, Users, Clock, Clipboard, Video, Play, CheckCircle2, XCircle, Save } from "lucide-react"
...
export default function MeetingDetailPage() {
  ...
  const handleUpdateSummary = async () => {
    try {
      ...
      setMeeting(meeting => meeting ? { ...meeting, summary: summaryText } : null)
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      <header className="bg-[#0a1e42] py-4 text-white sticky top-0 z-10">
        <div className="container mx-auto px-4">
          <div className="flex items-center">
            <Link href={`/discussion/${ideaId}/workplace`} className="mr-4 hover:text-gray-200 flex items-center">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to Workplace
            </Link>
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
          <div className="flex flex-wrap md:flex-nowrap justify-between items-start gap-4 mb-4">
            <div>
              <h2 className="text-2xl font-bold text-[#0a1e42]">{meeting.title}</h2>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2">
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${getMeetingStatusBadgeClass(meeting.status)}`}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <TabsContent value="meeting" className="space-y-4">
            <div className="bg-white rounded-lg shadow-sm p-4">
              <h3 className="text-lg font-semibold text-[#0a1e42] mb-4">Meeting Room</h3>
              {meeting.status === "CANCELLED" ? (
                <div className="bg-gray-50 border border-dashed rounded-lg p-8 text-center">
                  <XCircle className="h-12 w-12 text-red-400 mx-auto mb-4" />
                  <h3 className="text-lg font-medium text-gray-800 mb-2">This meeting has been cancelled</h3>
                  <p className="text-gray-600">The meeting room is not available for cancelled meetings.</p>
                </div>
              ) : meeting.status === "COMPLETED" ? (
                <div className="bg-gray-50 border border-dashed rounded-lg p-8 text-center">
                  <CheckCircle2 className="h-12 w-12 text-green-400 mx-auto mb-4" />
                  <h3 className="text-lg font-medium text-gray-800 mb-2">This meeting has ended</h3>
                  <p className="text-gray-600 mb-4">The meeting room is no longer active.</p>
                  {meeting.summary ? (
          ...
          <TabsContent value="participants" className="space-y-4">
            <div className="bg-white rounded-lg shadow-sm p-4">
              <h3 className="text-lg font-semibold text-[#0a1e42] mb-4">Participants ({meeting.participants.length})</h3>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Join Time</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Leave Time</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {meeting.participants.map((participant) => (
                      <tr key={participant.id}>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{participant.user.name}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{participant.user.email}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                          {participant.isPresenter ? (
                            <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full">Presenter</span>
                          ) : (
                            <span>Participant</span>
                          )}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                          {participant.joinTime ? formatDate(participant.joinTime) : "Not joined yet"}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                          {participant.leaveTime ? formatDate(participant.leaveTime) : "Still active"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Dialog open={isEditingSummary} onOpenChange={setIsEditingSummary}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{meeting?.summary ? "Edit Meeting Summary" : "Add Meeting Summary"}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEditingSummary(false)}>Cancel</Button>
            <Button onClick={handleUpdateSummary} disabled={!summaryText.trim()}>
