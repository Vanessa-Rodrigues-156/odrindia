...
export default function AdminIdeaApprovalPage() {
  ...
  const approveIdea = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Idea approved!",
        description: "The idea has been approved and is now live on the ODR Labs page.",
        variant: "default",
      });
    } catch (error) {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to approve idea. Please try again.",
        variant: "destructive",
      });
    } finally {
      setApproving(null);
    ...
    <div className="container mx-auto py-8">
      ...
      >
        <div className="mb-8 flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
          <div>
            <h1 className="mb-1 text-2xl font-bold text-[#0a1e42]">Idea Submission Review</h1>
            <p className="text-gray-500">Review and approve submitted ideas to make them public</p>
          </div>
            ...
            <div className="max-h-[70vh] overflow-y-auto p-4">
              {filteredSubmissions.length === 0 ? (
                <div className="flex h-40 flex-col items-center justify-center rounded-lg text-center">
                  ...
                  <p className="text-lg font-medium text-gray-500">No pending submissions found</p>
                  <p className="text-sm text-gray-400">
                    {searchTerm ? "Try adjusting your search criteria" : "All submissions have been reviewed"}
                  </p>
                </div>
              ) : (
                        ...
                        <Card className="overflow-hidden border-l-4 border-l-[#0a1e42] hover:shadow-md">
                          <div className="grid grid-cols-1 md:grid-cols-12 md:divide-x">
                            <div className="col-span-9 space-y-4 p-5">
                              <div>
                                <Badge variant="info" className="mb-2">ID: {submission.id.slice(0, 8)}</Badge>
                                <CardTitle className="mb-1 text-xl">{submission.title}</CardTitle>
                                {submission.ideaCaption && (
                                  <CardDescription>{submission.ideaCaption}</CardDescription>
                                )}
                              </div>
                              
                              <div className="space-y-3">
                                <div>
                                  <h4 className="mb-1 font-medium text-[#0a1e42]">Description</h4>
                                  <p className="text-gray-600">{submission.description}</p>
                                </div>
                                <div>
                                  <h4 className="mb-1 font-medium text-[#0a1e42]">ODR Experience</h4>
                                  <p className="text-gray-600">{submission.odrExperience}</p>
                                </div>
                              </div>                                <div className="flex flex-wrap items-center gap-3 pt-2 text-sm">
            ...
            <>
              <DialogHeader>
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-sm">
                  ...
                  <Badge variant="outline">Submitted: {format(new Date(selectedSubmission.createdAt), "PPp")}</Badge>
                </div>
              </DialogHeader>
              
              <div className="my-4 space-y-6">
                <div>
                  <h3 className="mb-2 font-medium text-[#0a1e42]">Description</h3>
                  <div className="rounded-md bg-gray-50 p-4 text-gray-700">
                    {selectedSubmission.description.split('\n').map((paragraph, i) => (
                ...
                <div>
                  <h3 className="mb-2 font-medium text-[#0a1e42]">ODR Experience</h3>
                  <div className="rounded-md bg-gray-50 p-4 text-gray-700">
                    {selectedSubmission.odrExperience.split('\n').map((paragraph, i) => (
                      <p key={i} className={i > 0 ? "mt-3" : ""}>{paragraph}</p>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div>
                    <h3 className="mb-2 font-medium text-[#0a1e42]">Submission Details</h3>
                    <div className="rounded-md bg-gray-50 p-4 text-sm">
                      <dl className="space-y-2">
                        <div className="flex justify-between">
                          <dt className="font-medium text-gray-500">Status:</dt>
                          <dd>
                            <Badge variant={selectedSubmission.approved ? "success" : "info"}>
                              {selectedSubmission.approved ? "Approved" : "Pending"}
                            </Badge>
                          </dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="font-medium text-gray-500">Consent Given:</dt>
                          <dd>{selectedSubmission.consent ? "Yes" : "No"}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="font-medium text-gray-500">Submission Date:</dt>
                          <dd>{format(new Date(selectedSubmission.createdAt), "PPp")}</dd>
                        </div>
                      </dl>
                    </div>
                  </div>
