      ...
      <div className="flex-grow overflow-y-auto mb-4 space-y-4">
        {loading ? (
          ...
          <div className="text-center p-6 bg-gray-50 rounded-lg border border-dashed">
            ...
            <p className="text-gray-500 text-sm">Add your first note to capture important information.</p>
          </div>
        ) : (
          notes.map((note) => (
            <div key={note.id} className="p-3 border rounded-lg bg-white shadow-sm">
              <div className="flex items-center mb-2">
                ...
                <div className="ml-2">
                  ...
                  <div className="text-xs text-gray-500">{formatNoteDate(note.createdAt)}</div>
                </div>
              </div>
              ...
              {note.lastEditedBy && note.updatedAt !== note.createdAt && (
                <div className="text-xs text-gray-500 mt-2 italic">
                  Edited {formatNoteDate(note.updatedAt)} by {note.lastEditedBy.name}
                </div>
              )}
            </div>
          ))
